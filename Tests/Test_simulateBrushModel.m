...
re = 0.5 * 15 * 25.4 + 0.55 * 195; % 195 / 55 R15
alpha = deg2rad(0);
...
data_range = 0.01 / (re/100);% 1 m/s; 3.6 km/h         %1.522137451171875e+02;
% data_range_2 = 0.1 / 3; % 10m/s; 36 km/h
% data_range_3 = 0.2 / 3; % 20 m/s; 72 km/h
omega(:, 1) = data_range * smootherstep(edge0, edge1, t_save) .* (1 - smootherstep(edge2, edge3, t_save));
omega(:, 2) = omega(:, 1) * 5;
omega(:, 3) = omega(:, 2) * 2;
SR = 0.1;
v0 = omega * re / (SR + 1);

% % data_range = -1.0 ;% m/s         %1.522137451171875e+02;
% % data_range_2 = 5.0; % m/s
% % data_range_3 = 10.0; % m/s
% % v0(:, 1) = data_range * smootherstep(edge0, edge1, t_save) .* (1 - smootherstep(edge2, edge3, t_save));
% % v0(:, 2) = v0(:, 1) * data_range_2;
% % v0(:, 3) = v0(:, 1) * data_range_3;
...
numElems = uint16(numBrushes);
LenTime_sim = (fs_sim * t_final + 1);
LenTime_save = (fs_save * t_final + 1);
dt_ratio = dt_save / dt_sim;

% load('Sliding_Pressure_fs_sim_1e5_fs_save_1e3_len_3s.mat')
load("C:\Users\coxde\OneDrive\Masters\BrushV2\TM700 Pressure Distribution\TM700Fz560Tr0_Buff_r1_SubSampled_20x20.mat")
...
for i = 1:K
    sim_solution{i} = simulateBrushModel_V2(numElems, LenTime_sim, LenTime_save, ...
                                                        Press, dt_sim, dt_save, omega(:, i), ...
                                                        SR, re, alpha, omega_z, X, Y);
end
...
% % 
% % for i = 1:3
% %     [Y_mag2(i, :), ~] = One_sided_fft(2 * Fz * v0(:, i) / max(v0(:, i), [], "all"), fs_sim);
% %     N = 2 * length(Y_mag2(i, :));
% % 
% %     PSD(i, :) = (1 /(fs_sim * N)) * real(Y_mag2(i, :)  .* conj(Y_mag2(i, :) ));
% % 
...
plot(t_save, (forceX));
hold on
% plot(t_save, forceX_medfilt)
% plot(t_save, forceX_filt)
grid on;
title("Longitudinal Force [N]")
% legend('Simulated', "Median Filtered", 'Median + MA Filtered')
legend(lgd2)
hold off
xlabel('Time [s]');ylabel('Force [N]')
ylim([0, 2 * Fz])
...
plot(t_save, forceY);
% hold on
% plot(t_save, forceY_medfilt)
% plot(t_save, forceY_filt)
grid on;
...
plot(t_save, forceTotal);
hold on
% plot(t_save, forceTotal_medfilt)
% plot(t_save, forceTotal_filt);
plot(t_save, (avg_mu * Fz), '--');
title('Total Force')
% legend("Total Force (Simulated from stresses) [N]",'Total Force (Median Filtered) [N]', "Total Force (Median + MA Filtered) [N]", "Avg Friction times Vertical Force [N]")
legend(lgd2)
grid on
ylim([0, 2 * Fz])
hold off
...
% % figure
% % plot(t_save, hypot(forceX_medfilt, forceY_medfilt));
% % hold on
% % plot(t_save, forceTotal_medfilt)
% % plot(t_save, (avg_mu * Fz));
% % grid on;
% % title("Total Force: Expected vs Calculated")
% % legend("Total Force (from forces) [N]", 'Total Force (Median Filtered) [N]', "Avg Friction times Vertical Force [N]")
% % ylim([0, 2 * Fz])
% % hold off
% % xlabel('Time [s]');ylabel('Force [N]')
...
% %     N = 2 * length(Y_mag2);
% % 
% %     PSD(i, :) = (1 /(fs_save * N)) * real(Y_mag2(i, :)  .* conj(Y_mag2(i, :) ));
% % end
% % figure
...
% % 
% % for i = 1:3
% % [Y_mag2(i, :), ~] = One_sided_fft(forceTotal_medfilt(i, :), fs_save);
% % N = 2 * length(Y_mag2(i, :));
% % 
% % PSD(i, :) = (1 /(fs_save * N)) * real(Y_mag2(i, :)  .* conj(Y_mag2(i, :) ));
% % end
...
% % 
% % for i = 1:3
% %     [Y_mag2(i, :), ~] = One_sided_fft(forceTotal_filt(i, :), fs_save);
% %     N = 2 * length(Y_mag2(i, :));
% % 
% %     PSD(i, :) = (1 /(fs_save * N)) * real(Y_mag2(i, :)  .* conj(Y_mag2(i, :) ));
% % end
...
if size(plot_solution, 1) ~= numBrushes 
    % sim_sol_filtered = medfilt3(sim_solution);
    % P_grid_save = reshape(P_grid.save, numBrushes, numBrushes, LenTime_save);
    % sim_solution = reshape(sim_solution, numBrushes, numBrushes, LenTime_save, 8);
    % plot_solution = cat(3, P_grid.save, sim_solution(:, :, 1:8) .* pressure_mask);%, bool_array(:, :, 1) .* pressure_mask);
    plot_solution = reshape(working_data, numBrushes, numBrushes, LenTime_save, 9);
    % clear P_grid_save sim_solution
end
...
% % video_filename = 'Interpolated_Pressure';
...
for t = plot_ind
    ...
    % % if t == 527
    % %     for j = 1:9
    % %         set(h(j), 'CLim', clim_values2(j, :));
    % %     end
    % % 
    % % elseif t == 527+1009
    % %     for j = 1:9
    % %         set(h(j), 'CLim', clim_values3(j, :));
    % %     end
    % % elseif t == 527 + 1009 + 2000
    % %     for j = 1:9
    % %         set(h(j), 'CLim', clim_values4(j, :));
    % %     end
    % % end
