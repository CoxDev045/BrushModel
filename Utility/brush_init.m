function [model_input] = brush_init(numBrushes, isRolling, fs_sim, fs_save, t_initial, t_final)
    ...
    if isRolling
        ...
        % LenTime_sim = ((edge2 - edge1) / 2);
        % time_ind = ((edge1 +1 ) * fs_sim):LenTime_sim;
        
        ramp = rampFunc(edge1, edge2, t_sim);
        % % % Extend ramp downwards
        % % ramp = zeros(size(t_sim));
        % % ramp_ind = ((edge1 +1 ) * fs_sim):((edge2 -1 ) * fs_sim);
        % % ramp(ramp_ind) = [TempRamp(1:end-1), fliplr(TempRamp)];

        model_input.SR = single( (ramp(:)) );
